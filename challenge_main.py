...
y = fc_layer(hidden13, [3 * 3 * filter_nb_3, template_dim], 'fc_final', keep_prob, act=None)
...
def feed_func(batch_size, mode='train', placeholder_dict=placeholder_dict, cursors=cursors):
	...
	if mode == 'train':
		...
		ind_batch = np.mod(np.arange(tmp_cur, tmp_cur + batch_size), nb_img_train).astype(int)
		...
		ind_batch = np.mod(np.arange(tmp_cur, tmp_cur + batch_size), nb_img_valid)
...
while i < nb_max_iter:
	...
	if ((np.mod(i, validation_log_frequency_iter) == 0) & (not i == 0)):
		...
		for jj in np.arange(nb_iter_validation):
			...
			ind_tmp = np.mod(np.arange(jj * batch_test, (jj + 1) * batch_test), nb_img_valid).astype(int)
